...
let ScraperService = ScraperService_1 = class ScraperService {
    constructor(fruitRepository, marketRepository, lotRepository, userRepository, priceHistoryRepository) {
    ...
    async ensureReferenceCatalog() {
        ...
        const existingFruitNames = new Set(fruits.map((fruit) => this.normalizeText(fruit.nameEs)));
        const missingFruits = ScraperService_1.defaultSeedFruits.filter((seed) => !existingFruitNames.has(this.normalizeText(seed.nameEs)));
        ...
        const missingMarkets = ScraperService_1.defaultSeedMarkets.filter((seed) => {
    ...
    async cleanupLegacyReferencePrices() {
        await this.priceHistoryRepository
            .createQueryBuilder()
            .delete()
            .from(price_history_entity_1.PriceHistory)
            .where(`additionalData ->> 'source' = :source`, { source: 'Mercosur Open Data' })
            .execute();
    }
    async replaceReferencePricesForSource(source, sourceRegion, entries) {
        if (entries.length === 0) {
            return;
        }
        await this.priceHistoryRepository
    ...
    async cleanupDuplicateMarkets() {
        const markets = await this.marketRepository.find({ where: { active: true } });
        for (const [canonicalName, aliases] of Object.entries(ScraperService_1.marketAliasMap)) {
            const normalizedNames = new Set([canonicalName, ...aliases].map((value) => this.normalizeText(value)));
            ...
            for (const countryMarkets of groupedByCountry.values()) {
                ...
                const canonicalMarket = countryMarkets.find((market) => this.normalizeText(market.name) === this.normalizeText(canonicalName)) ??
                    countryMarkets.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime())[0];
    ...
    async syncEuReferencePrices(fruits, markets) {
        let saved = 0;
        let attempted = 0;
        let matched = 0;
        ...
        for (const fruit of fruits) {
            ...
            for (const market of markets) {
                ...
                if (!code || code === 'US' || !ScraperService_1.euCountryCodes.has(code))
                    continue;
    ...
    async syncUsdaReferencePrices(fruits, markets) {
        ...
        const usaMarkets = markets.filter((market) => this.resolveCountryCode(market.country) === 'US');
    ...
    async syncMercosurReferencePrices(fruits, markets) {
        const diagnostics = {
        ...
        for (const market of markets) {
            const code = this.resolveCountryCode(market.country);
            if (!code || !['AR', 'BR', 'CL', 'PY', 'UY'].includes(code))
                continue;
            const bucket = marketsByCountry.get(code) ?? [];
            bucket.push(market);
            marketsByCountry.set(code, bucket);
        ...
        if (argentinaMarkets.length > 0) {
            ...
            if (argentinaRows.length === 0) {
                ...
                diagnostics.notes.push(`Mercado Central rows: ${argentinaRows.length}, latest ${argentinaRows[0]?.referenceDate.toISOString().slice(0, 10) ?? 'n/a'}`);
            }
        }
        else {
        ...
        if (brazilMarkets.length > 0) {
            ...
            if (brazilRows.length === 0) {
                ...
                diagnostics.notes.push(`CEAGESP rows: ${brazilRows.length}, latest ${brazilRows[0]?.referenceDate.toISOString().slice(0, 10) ?? 'n/a'}`);
            }
        }
        else {
    ...
    async syncCanadaReferencePrices(fruits, markets) {
        const diagnostics = {
    ...
    async syncMexicoReferencePrices(fruits, markets) {
        const diagnostics = {
        ...
        for (const fruit of fruits) {
            ...
            for (let offset = 0; offset < 3; offset += 1) {
                ...
                const averageFrequent = Number((frequentValues.reduce((total, value) => total + value, 0) / frequentValues.length).toFixed(2));
                matchedPrice = {
            ...
            for (const market of mexicoMarkets) {
                pendingUpserts.push({
                    ...
                    metadata: {
                        ...
                        sniimDestinationId: ScraperService_1.sniimDestinationId,
                        sniimDestination: ScraperService_1.sniimDestinationLabel,
    ...
    async syncAsiaReferencePrices(fruits, markets) {
        const diagnostics = {
        ...
        for (const fruit of fruits) {
            ...
            const modalAverage = latestRows.reduce((total, row) => total + row.modalPrice, 0) / latestRows.length;
    ...
    async syncAustraliaReferencePrices(fruits, markets) {
        const diagnostics = {
    ...
    async syncArabReferencePrices(fruits, markets) {
        const diagnostics = {
        ...
        for (const fruit of fruits) {
            ...
            diagnostics.attempted += 1;
            const matchedRows = rows.filter((row) => aliases.some((alias) => this.normalizeArabicText(row.product).includes(alias)));
            if (matchedRows.length === 0) {
                diagnostics.notes.push(`No Obour price for ${fruit.nameEs}`);
                continue;
            }
            const averagePrice = matchedRows.reduce((total, row) => total + (row.minPrice + row.maxPrice) / 2, 0) /
                matchedRows.length;
            ...
            const latestReferenceDate = matchedRows.find((row) => row.referenceDate.getTime() === latestTimestamp)?.referenceDate;
    ...
    async fetchLatestEuPrice(product, memberStateCode, currentYear) {
    ...
    async fetchLatestEuQlikPrice(product, memberStateCode, currentYear) {
        try {
            const result = await this.queryEuQlikRows(product, memberStateCode, currentYear);
            if (result.rows.length === 0) {
                this.logger.log(result.debugNote ||
                    `EU Qlik returned 0 rows for ${product}/${memberStateCode}`);
                return null;
            }
            const latest = result.rows.sort((a, b) => b.referenceDate.getTime() - a.referenceDate.getTime())[0];
    ...
    async fetchLatestEuLegacyPrice(product, memberStateCode, currentYear) {
        try {
            ...
            for (const productVariant of productVariants) {
                try {
                    const response = await axios_1.default.get('https://agridata.ec.europa.eu/api/fruitAndVegetable/pricesSupplyChain', {
                        timeout: 15000,
                        params: {
                    ...
                    const status = axios_1.default.isAxiosError(variantError)
            ...
            this.logger.warn(`EU reference fetch failed for ${product}/${memberStateCode}: ${status ?? 'no-status'} ${error.message}`);
            return null;
        }
    }
    async fetchLatestUsdaRetailPrice(apiKey, commodity) {
        try {
            const auth = Buffer.from(`${apiKey}:`).toString('base64');
            const response = await axios_1.default.get('https://marsapi.ams.usda.gov/services/v1.2/reports/3324/Details', {
                timeout: 15000,
                headers: {
            ...
            if (rows.length === 0) {
                const keys = response.data != null && typeof response.data === 'object'
            ...
            const parsed = rows
                .map((row) => {
                const rawPrice = row.weighted_average ??
                    row.weighted_avg ??
                    row.weightedaverage ??
                    row.average_price ??
                    row.average ??
                ...
                const dateValue = row.published_date ??
                    row.report_date ??
                    row.report_end_date ??
                ...
                .filter((row) => this.matchesCommodity(row.commodity, commodity) &&
            ...
            const status = axios_1.default.isAxiosError(error) ? error.response?.status : undefined;
            this.logger.warn(`USDA reference fetch failed for ${commodity}: ${status ?? 'no-status'} ${error.message}`);
            return null;
        }
    }
    async debugEuReferenceSource(product, memberStateCode, currentYear) {
        try {
            const result = await this.queryEuQlikRows(product, memberStateCode, currentYear);
            return {
                provider: 'EU AgriData',
            ...
            const status = axios_1.default.isAxiosError(error) ? error.response?.status : undefined;
            const payload = axios_1.default.isAxiosError(error) ? error.response?.data : undefined;
            return {
                provider: 'EU AgriData',
                ok: false,
                status,
    ...
    async fetchMercadoCentralRows() {
        try {
            const indexResponse = await axios_1.default.get(ScraperService_1.mercadoCentralIndexUrl, {
                timeout: 20000,
                responseType: 'text',
            });
            ...
            for (const zipUrl of zipUrls) {
                try {
                    ...
                    this.logger.warn(`Mercado Central zip parsing failed for ${zipUrl}: ${zipError.message}`);
                }
            }
            return collected.sort((a, b) => b.referenceDate.getTime() - a.referenceDate.getTime());
        }
        catch (error) {
            const status = axios_1.default.isAxiosError(error) ? error.response?.status : undefined;
            this.logger.warn(`Mercado Central fetch failed: ${status ?? 'no-status'} ${error.message}`);
            return [];
        }
    }
    extractMercadoCentralZipUrl(html, prefix) {
        const matches = Array.from(html.matchAll(new RegExp(`https?:\\/\\/mercadocentral\\.gob\\.ar[^"'\\s>]*${prefix}[^"'\\s>]*\\.zip`, 'gi')))
    ...
    extractMercadoCentralMonthScore(url) {
        ...
        const month = Object.entries(months).find(([key]) => normalized.includes(key))?.[1] ?? 0;
        return year * 100 + month;
    }
    async fetchMercadoCentralZipRows(zipUrl) {
        const response = await axios_1.default.get(zipUrl, {
            timeout: 30000,
        ...
        const entry = zip
            ...
            .sort((a, b) => this.extractMercadoCentralSheetDate(b.entryName).getTime() -
            this.extractMercadoCentralSheetDate(a.entryName).getTime())[0];
        if (!entry) {
            return [];
        }
        ...
        const headerIndex = rows.findIndex((row) => row.some((cell) => this.normalizeText(`${cell}`).includes('esp')));
        if (headerIndex < 0) {
            return [];
        }
        const header = rows[headerIndex].map((cell) => this.normalizeText(`${cell}`));
        const getIndex = (name) => header.findIndex((cell) => cell === name);
    ...
    extractMercadoCentralSheetDate(filename) {
    ...
    selectLatestMercadoCentralPrice(rows, aliases) {
        const preferred = rows.filter((row) => aliases.some((alias) => {
    ...
    async fetchCeagespRows() {
        try {
            const indexResponse = await axios_1.default.get(ScraperService_1.ceagespPricesUrl, {
                timeout: 20000,
                responseType: 'text',
            });
            const html = `${indexResponse.data ?? ''}`;
            const groups = ['FRUTAS', 'LEGUMES', 'VERDURAS'];
            const collected = [];
            for (const group of groups) {
                try {
                    ...
                    const response = await axios_1.default.post(ScraperService_1.ceagespPricesUrl, payload.toString(), {
                        timeout: 20000,
                        responseType: 'text',
                        headers: {
                    ...
                    this.logger.warn(`CEAGESP parsing failed for ${group}: ${groupError.message}`);
                }
            }
            return collected.sort((a, b) => b.referenceDate.getTime() - a.referenceDate.getTime());
        }
            ...
            const status = axios_1.default.isAxiosError(error) ? error.response?.status : undefined;
            this.logger.warn(`CEAGESP fetch failed: ${status ?? 'no-status'} ${error.message}`);
            return [];
        }
    }
    extractCeagespLatestDate(html, group) {
        const blockMatch = html.match(new RegExp(`"${group}"\\s*:\\s*\\[([^\\]]+)\\]`, 'i'));
        if (!blockMatch) {
            return null;
        }
        const dates = Array.from(blockMatch[1].matchAll(/(\d{2}\\?\/\d{2}\\?\/\d{4})/g)).map((match) => match[1].replace(/\\\//g, '/'));
        if (dates.length === 0) {
            return null;
        }
        return dates.sort((a, b) => this.parseCeagespDate(b).getTime() - this.parseCeagespDate(a).getTime())[0];
    }
    parseCeagespTable(html, dateValue) {
        const referenceDate = this.parseCeagespDate(dateValue);
        const rows = Array.from(html.matchAll(/<tr[^>]*>([\s\S]*?)<\/tr>/gi));
        const collected = [];
        for (const rowMatch of rows) {
            const cells = Array.from(rowMatch[1].matchAll(/<t[dh][^>]*>([\s\S]*?)<\/t[dh]>/gi)).map((match) => this.decodeHtmlEntities(match[1]).replace(/\s+/g, ' ').trim());
    ...
    selectLatestCeagespPrice(rows, aliases) {
        const preferred = rows.filter((row) => aliases.some((alias) => {
        ...
        const latest = preferred.sort((a, b) => b.referenceDate.getTime() - a.referenceDate.getTime())[0];
    ...
    async fetchMercosurRows(url, countryCode) {
        try {
            ...
            const status = axios_1.default.isAxiosError(error) ? error.response?.status : undefined;
            this.logger.warn(`Mercosur reference fetch failed for ${countryCode}: ${status ?? 'no-status'} ${error.message}`);
            return [];
        }
    }
    resolveMercosurProductAliases(nameEs) {
        const normalized = this.normalizeText(nameEs);
        for (const [key, aliases] of Object.entries(ScraperService_1.mercosurProductMap)) {
    ...
    selectLatestMercosurPrice(rows, aliases) {
        const parsed = rows
            .map((row) => {
            ...
            const referenceDate = this.parseMercosurDate(this.extractMercosurYearValue(row), `${row.mes ?? ''}`);
            return {
                product,
                searchable,
            ...
            .filter((row) => row.referenceDate != null &&
            !Number.isNaN(row.price) &&
            row.price > 0 &&
            aliases.some((alias) => row.searchable.includes(alias) || alias.includes(row.searchable)))
            .sort((a, b) => (b.referenceDate?.getTime() ?? 0) - (a.referenceDate?.getTime() ?? 0));
        if (parsed.length === 0) {
            return null;
        }
    ...
    async debugUsdaReferenceSource(commodity) {
        const apiKey = process.env.USDA_MARS_API_KEY?.trim();
        if (!apiKey) {
            return {
                provider: 'USDA Market News',
    ...
    async upsertReferencePrice(input) {
        ...
        const entry = existing ?? new price_history_entity_1.PriceHistory();
    ...
    resolveEuProductName(nameEs) {
        const normalized = this.normalizeText(nameEs);
        for (const [key, value] of Object.entries(ScraperService_1.euProductMap)) {
            if (normalized.includes(key)) {
                return value;
            }
        }
    ...
    async queryEuQlikRows(product, memberStateCode, currentYear) {
        ...
        try {
            ...
            for (const stage of ScraperService_1.euQlikStagePriority) {
                for (const productPattern of productPatterns) {
                    ...
                    const measureExpression = `=Avg({<[Member State Code]={'${safeCountry}'},` +
    ...
    async fetchEuQlikAppId() {
        try {
            const response = await axios_1.default.get(ScraperService_1.euQlikAppIdMappingUrl, {
                timeout: 15000,
            });
            const rows = Array.isArray(response.data) ? response.data : [];
            const mapped = rows.find((row) => `${row?.name ?? ''}`.trim() === 'Fruits and Vegetables - Supply Chain');
            return `${mapped?.id ?? ScraperService_1.euQlikDefaultAppId}`.trim();
        }
        catch (error) {
            this.logger.warn(`EU Qlik app-id mapping fetch failed: ${error.message}`);
            return ScraperService_1.euQlikDefaultAppId;
        }
    }
    resolveEuQlikProductPatterns(product) {
        const mapped = ScraperService_1.euQlikProductPatternMap[product];
        if (mapped && mapped.length > 0) {
            return mapped;
        }
        return [`${product.replace(/"/g, '').trim()}*`];
    }
    escapeQlikSetLiteral(value) {
        return `${value ?? ''}`.replace(/'/g, "''").trim();
    }
    escapeQlikSearchLiteral(value) {
        return `${value ?? ''}`.replace(/\\/g, '\\\\').replace(/"/g, '\\"').trim();
    }
    async openEuQlikSession() {
        const socket = new ws_1.default(ScraperService_1.euQlikWsUrl, {
            headers: {
        ...
        return {
            request: (handle, method, params) => new Promise((resolve, reject) => {
                const id = nextId++;
    ...
    extractUsdaRows(payload) {
        if (Array.isArray(payload)) {
            return payload.flatMap((item) => this.extractUsdaRows(item));
        }
        if (payload && typeof payload === 'object') {
            const looksLikeLeafRow = 'commodity' in payload ||
    ...
    matchesCommodity(searchableText, commodity) {
    ...
    resolveUsdaCommodityName(nameEs) {
        const normalized = this.normalizeText(nameEs);
        for (const [key, value] of Object.entries(ScraperService_1.usdaProductMap)) {
            if (normalized.includes(key)) {
                return value;
            }
        }
    ...
    resolveCountryCode(country) {
        ...
        for (const [key, value] of Object.entries(ScraperService_1.countryCodeMap)) {
            if (normalized.includes(key)) {
                return value;
            }
        }
    ...
    buildMarketIdentityKeys(name, country) {
        const normalizedName = this.normalizeText(name);
        const normalizedCountry = this.normalizeText(country);
        const keys = new Set([`${normalizedName}:${normalizedCountry}`]);
        for (const [canonicalName, aliases] of Object.entries(ScraperService_1.marketAliasMap)) {
    ...
    parseEuDate(value) {
        ...
        if ([day, month, year].some((item) => Number.isNaN(item)))
            return null;
        return new Date(Date.UTC(year, month - 1, day));
    }
    parseUsdaDate(value) {
    ...
    parseMercosurDate(yearValue, monthValue) {
    ...
    extractMercosurYearValue(row) {
        ...
        return dynamicKey ? `${row[dynamicKey] ?? ''}`.trim() : '';
    }
    ...
    resolveIndiaMandiCommodityLabels(nameEs) {
        const normalized = this.normalizeText(nameEs);
        for (const [key, value] of Object.entries(ScraperService_1.indiaMandiProductMap)) {
            if (normalized.includes(key)) {
                return value;
            }
        }
    ...
    resolveAustraliaDaffLabels(nameEs) {
        const normalized = this.normalizeText(nameEs);
        for (const [key, value] of Object.entries(ScraperService_1.australiaDaffProductMap)) {
            if (normalized.includes(key)) {
                return value;
            }
        }
    ...
    async fetchAustraliaDaffIndices() {
        try {
            const response = await axios_1.default.get(ScraperService_1.australiaDaffPricesUrl, {
                timeout: 40000,
                responseType: 'text',
            });
            ...
            for (const match of html.matchAll(/alt="([^"]+?)"/gi)) {
                ...
                const seriesMatch = altText.match(/(Apple prices in Melbourne|Banana prices in Melbourne|Orange prices in Melbourne|Strawberry prices in Melbourne|Onion prices in Melbourne|Potato prices in Melbourne|Tomato prices in Melbourne)[^0-9\-]*(-?\d+(?:\.\d+)?)/i);
                if (!seriesMatch) {
                    continue;
                }
    ...
    async fetchIndiaMandiRows(commodity) {
        try {
            const response = await axios_1.default.get(ScraperService_1.indiaMandiApiUrl, {
                timeout: 20000,
                params: {
                    'api-key': ScraperService_1.indiaMandiApiKey,
                    format: 'json',
                    offset: 0,
                    limit: 200,
    ...
    resolveObourProductAliases(nameEs) {
        const normalized = this.normalizeText(nameEs);
        for (const [key, value] of Object.entries(ScraperService_1.obourProductMap)) {
            if (normalized.includes(key)) {
    ...
    async fetchObourLatestCards() {
        try {
            const response = await axios_1.default.get(ScraperService_1.obourPricesUrl, {
                timeout: 20000,
                responseType: 'text',
            });
            const html = `${response.data ?? ''}`;
            ...
            const blocks = Array.from(html.matchAll(/<div class='card'[\s\S]*?<\/div><hr \/>/gi)).map((match) => match[0]);
            const rows = [];
            for (const block of blocks) {
                const product = this.decodeHtmlEntities(block.match(/<h5[^>]*>\s*([^<]+?)\s*<\/h5>/i)?.[1] ?? '')
                    .replace(/\s+/g, ' ')
                    .trim();
                const unitRaw = this.decodeHtmlEntities(block.match(/class='status'[^>]*>\s*([^<]+?)\s*<\/span>/i)?.[1] ?? '')
                    .replace(/\s+/g, ' ')
                    .trim();
                const packaging = this.decodeHtmlEntities(block.match(/العبوة:\s*<strong>\s*([^<]+?)\s*<\/strong>/i)?.[1] ?? '')
                    .replace(/\s+/g, ' ')
                    .trim();
                const minRaw = block.match(/من سعر\s*:\s*<strong>\s*([^<]+?)\s*<\/strong>/i)?.[1];
                const maxRaw = block.match(/الى سعر\s*:\s*<strong>\s*([^<]+?)\s*<\/strong>/i)?.[1];
    ...
    async fetchLatestStatsCanPrices() {
        try {
            const response = await axios_1.default.get(ScraperService_1.statsCanPricesUrl, {
                responseType: 'arraybuffer',
                timeout: 20000,
            });
            const zip = new AdmZip(Buffer.from(response.data));
            const entry = zip.getEntry(ScraperService_1.statsCanCsvName);
            if (!entry) {
            ...
            const supportedProducts = new Set(Object.values(ScraperService_1.statsCanProductMap).flat());
    ...
    resolveStatsCanProductLabels(nameEs) {
        const normalized = this.normalizeText(nameEs);
        for (const [key, value] of Object.entries(ScraperService_1.statsCanProductMap)) {
            if (normalized.includes(key)) {
                return value;
            }
        }
    ...
    resolveStatsCanUnitType(productLabel) {
        const normalized = productLabel.toLowerCase();
        if (normalized.includes('per kilogram'))
            return 'kg';
        if (normalized.includes('unit'))
            return 'unit';
        if (normalized.includes('454 grams'))
            return '454g';
        if (normalized.includes('4.54 kilograms'))
            return '4.54kg';
        return 'unit';
    }
    parseStatsCanDate(value) {
    ...
    resolveSniimProductId(nameEs) {
        const normalized = this.normalizeText(nameEs);
        for (const [key, value] of Object.entries(ScraperService_1.sniimProductMap)) {
            if (normalized.includes(key)) {
                return value;
            }
        }
    ...
    async fetchSniimRows(productId, referenceDate) {
        try {
            const response = await axios_1.default.get(ScraperService_1.sniimResultsUrl, {
                timeout: 15000,
                responseType: 'text',
                params: {
                    ...
                    OrigenId: -1,
                    Origen: 'Todos',
                    DestinoId: ScraperService_1.sniimDestinationId,
                    Destino: ScraperService_1.sniimDestinationLabel,
                    PreciosPorId: 2,
            ...
            for (let index = sectionIndex + 1; index <= tokens.length - 5; index += 1) {
                ...
                if (current.toUpperCase().startsWith('PRECIO MIN') ||
    ...
    normalizeUsdaUnit(unitRaw) {
        const normalized = unitRaw.toLowerCase();
        if (normalized.includes('lb'))
            return 'lb';
        if (normalized.includes('kg'))
            return 'kg';
        if (normalized.includes('ea'))
            return 'unit';
        if (normalized.includes('ct'))
            return 'unit';
        if (normalized.includes('bunch'))
            return 'bunch';
        return 'unit';
    }
    normalizeObourUnit(unitRaw) {
        const normalized = this.normalizeArabicText(unitRaw);
        if (normalized.includes('كيلو'))
            return 'kg';
        if (normalized.includes('طن'))
            return 't';
        if (normalized.includes('وحد'))
            return 'unit';
        return 'kg';
    }
    todayUtc() {
...
ScraperService.statsCanProductMap = {
...
ScraperService.sniimProductMap = {
    aguacates: '133',
    cebollas: '183',
    pimientos: '221',
    fresas: '358',
...
ScraperService.indiaMandiProductMap = {
    manzanas: ['Apple'],
    bananas: ['Banana'],
    naranjas: ['Orange'],
    limones: ['Lemon'],
...
ScraperService.obourProductMap = {
    naranjas: ['برتقال'],
    limones: ['ليمون'],
...
ScraperService.australiaDaffProductMap = {
...
ScraperService.mercosurProductMap = {
    naranjas: ['naranja'],
    limones: ['limon'],
...
ScraperService.mercosurSources = [
    {
        countryCode: 'AR',
...
ScraperService.mercadoCentralIndexUrl = 'https://mercadocentral.gob.ar/informaci%C3%B3n/precios-mayoristas';
ScraperService.ceagespPricesUrl = 'https://ceagesp.gov.br/cotacoes/';
ScraperService.statsCanPricesUrl = 'https://www150.statcan.gc.ca/n1/tbl/csv/18100245-eng.zip';
ScraperService.statsCanCsvName = '18100245.csv';
ScraperService.sniimResultsUrl = 'http://www.economia-sniim.gob.mx/Nuevo/Consultas/MercadosNacionales/PreciosDeMercado/Agricolas/ResultadosConsultaFechaFrutasYHortalizas.aspx';
ScraperService.sniimDestinationId = '100';
ScraperService.sniimDestinationLabel = 'DF: Central de Abasto de Iztapalapa DF';
ScraperService.indiaMandiApiUrl = 'https://api.data.gov.in/resource/9ef84268-d588-465a-a308-a864a43d0070';
ScraperService.indiaMandiApiKey = '579b464db66ec23bdd000001cdc3b564546246a772a26393094f5645';
ScraperService.obourPricesUrl = 'http://www.oboormarket.org.eg/prices_ar.aspx';
ScraperService.australiaDaffPricesUrl = 'https://www.agriculture.gov.au/abares/data/weekly-commodity-price-update/australian-horticulture-prices';
ScraperService.countryCodeMap = {
    spain: 'ES',
    espana: 'ES',
    espanaa: 'ES',
    france: 'FR',
...
ScraperService.euCountryCodes = new Set(['ES', 'FR', 'IT', 'DE', 'PT', 'NL']);
ScraperService.euQlikAppIdMappingUrl = 'https://agridata.ec.europa.eu/files/app-id-mapping.json';
ScraperService.euQlikDefaultAppId = 'b2e1f9c6-ba92-424e-82c6-2d18183b27d1';
ScraperService.euQlikWsUrl = 'wss://agridata.ec.europa.eu/app/engineData';
ScraperService.euQlikStagePriority = [
    'Retail buying price',
    'Ex-packaging station price',
    'Farmgate price',
];
ScraperService.euQlikProductPatternMap = {
    Apples: ['Apples*'],
    Oranges: ['Oranges*'],
    Lemons: ['Lemons*'],
...
ScraperService.defaultSeedFruits = [
    {
        nameEs: 'Naranjas',
        nameEn: 'Oranges',
        nameFr: 'Oranges',
...
ScraperService.defaultSeedMarkets = [
