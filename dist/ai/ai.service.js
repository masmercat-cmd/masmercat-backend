...
let AiService = class AiService {
    ...
    normalizePriceTiers(value) {
        ...
        return value
            ...
            const min = Math.max(1, Math.round(this.toNumber(entry.min_palets ?? entry.min ?? 0)));
            const maxRaw = this.toNumber(entry.max_palets ?? entry.max ?? 0);
            const max = maxRaw > 0 ? Math.round(maxRaw) : null;
            const price = this.toNumber(entry.price_per_pallet ?? entry.price ?? 0);
    ...
    resolvePricePerPallet(parsed, palletCount) {
    ...
    finalizeTransportTariffResult(parsed, raw, origin, destination, palletCount, palletType, lang) {
        ...
        const inferredCarrier = parsedCarrier ||
        ...
        const inferredIndustrialMultiplier = inferredCarrier.toLowerCase().includes('castillo') &&
...
- If the tariff belongs to a company like Castillo, return "Castillo" in carrier.
	...
	`;
        ...
        return this.finalizeTransportTariffResult(parsed, raw, origin, destination, palletCount, palletType, lang);
    }
